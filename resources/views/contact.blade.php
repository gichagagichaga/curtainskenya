...
@section('title', 'Contact Us | Curtains Kenya')

@section('description', 'Contact Curtains Kenya for product enquiries, custom requirements and home textile advice.')
    ...
    <div class="mx-auto grid max-w-7xl gap-10 px-6 py-14 lg:grid-cols-[1.15fr_0.85fr] lg:px-8 lg:py-20">
        <div>
            ...
            <div class="mt-12 border-t border-[#e7dfd5] pt-10">
                ...
                @if (session('status'))
                    <div class="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{{ session('status') }}</div>
                @endif

                <form method="POST" action="{{ route('contact.store') }}" class="mt-7 grid gap-5 sm:grid-cols-2">
                    @csrf
                    <div><label for="name" class="text-sm font-medium text-[#29231e]">Name</label><input id="name" name="name" type="text" value="{{ old('name') }}" required autocomplete="name" class="mt-2 block w-full rounded-lg border-[#d8cfc4] bg-white text-[#29231e] shadow-sm focus:border-[#8a6a4a] focus:ring-[#8a6a4a]">@error('name')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                    <div><label for="email" class="text-sm font-medium text-[#29231e]">Email address</label><input id="email" name="email" type="email" value="{{ old('email') }}" required autocomplete="email" class="mt-2 block w-full rounded-lg border-[#d8cfc4] bg-white text-[#29231e] shadow-sm focus:border-[#8a6a4a] focus:ring-[#8a6a4a]">@error('email')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                    <div><label for="phone" class="text-sm font-medium text-[#29231e]">Phone number <span class="font-normal text-[#8b7d70]">(optional)</span></label><input id="phone" name="phone" type="tel" value="{{ old('phone') }}" autocomplete="tel" class="mt-2 block w-full rounded-lg border-[#d8cfc4] bg-white text-[#29231e] shadow-sm focus:border-[#8a6a4a] focus:ring-[#8a6a4a]">@error('phone')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                    <div><label for="subject" class="text-sm font-medium text-[#29231e]">Subject</label><input id="subject" name="subject" type="text" value="{{ old('subject') }}" required class="mt-2 block w-full rounded-lg border-[#d8cfc4] bg-white text-[#29231e] shadow-sm focus:border-[#8a6a4a] focus:ring-[#8a6a4a]">@error('subject')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                    <div class="sm:col-span-2"><label for="message" class="text-sm font-medium text-[#29231e]">How can we help?</label><textarea id="message" name="message" rows="6" required class="mt-2 block w-full rounded-lg border-[#d8cfc4] bg-white text-[#29231e] shadow-sm focus:border-[#8a6a4a] focus:ring-[#8a6a4a]">{{ old('message') }}</textarea>@error('message')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror</div>
