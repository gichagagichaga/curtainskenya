<form method="POST" action="{{ $product ? route('admin.products.update', $product) : route('admin.products.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
    ...
    <div class="grid gap-6 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900 sm:p-6 lg:grid-cols-2">
        ...
        <div><label for="color" class="text-sm font-medium text-zinc-900 dark:text-white">Product color <span class="font-normal text-zinc-500">(optional)</span></label><input id="color" name="color" type="text" maxlength="100" value="{{ old('color', $product?->color) }}" placeholder="e.g. Ivory, Navy Blue" class="mt-2 block w-full rounded-lg border-zinc-300 bg-white text-zinc-900 shadow-sm placeholder:text-zinc-400 focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white dark:placeholder:text-zinc-500 dark:focus:border-white dark:focus:ring-white">@error('color')<p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror</div>
