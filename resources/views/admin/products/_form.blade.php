<form method="POST" action="{{ $product ? route('admin.products.update', $product) : route('admin.products.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
    ...
    <div class="grid gap-6 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900 sm:p-6 lg:grid-cols-2">
        ...
        <div>
            <label for="color" class="text-sm font-medium text-zinc-900 dark:text-white">Product color <span class="font-normal text-zinc-500">(optional)</span></label>
            <input id="color" name="color" type="text" list="product-colors" maxlength="100" value="{{ old('color', $product?->color) }}" placeholder="Choose or type a color" autocomplete="off" class="mt-2 block w-full rounded-lg border-zinc-300 bg-white text-zinc-900 shadow-sm placeholder:text-zinc-400 focus:border-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white dark:placeholder:text-zinc-500 dark:focus:border-white dark:focus:ring-white">
            <datalist id="product-colors">
                @foreach (['Beige', 'Black', 'Blue', 'Brown', 'Burgundy', 'Cream', 'Gold', 'Green', 'Grey', 'Ivory', 'Maroon', 'Navy Blue', 'Orange', 'Pink', 'Purple', 'Red', 'Silver', 'Teal', 'White', 'Yellow'] as $color)
                    <option value="{{ $color }}"></option>
                @endforeach
            </datalist>
            <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">Choose a suggested color or type a different one.</p>
